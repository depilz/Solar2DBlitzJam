local TaskQueue = require("Utils.taskQueue")
...
function Time.enterFrame()
  ...
  local timeElapsed = min(currentTime - _prevTime, 50) * Time.scale
  ...
  for i=1, #_enterFrameObjects do
    if not newUnsubscribers[_enterFrameObjects[i]] then
...
function Time.resume(avoidPause)
  if not Time.isPaused then return false end
  if avoidPause then
