...
-- Remove the file and its backup.
function loadsave.remove( filename, directory )
    directory = directory or system.DocumentsDirectory
    local result, reason = os.remove( system.pathForFile( filename, directory ) )
    result, reason = os.remove( system.pathForFile( "backup_"..filename, directory ) )
    
    if result then
        print( "LoadSave: "..filename.." removed" )
    else
        print( "LoadSave: error removing file - "..tostring(reason))
    end
end
